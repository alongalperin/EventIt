...
let status;
let apiKey = "AIzaSyC6lDxO8tsmlTKYGCFZmTxT9wc_uNVeRx0";
...
const app = new Vue({
  ...
  methods: {
    ...
    changeCounter: function (song) {
      let songId = song.id;
      if (song.isUserLiked === false) {
        // user add like song
        // /POST /event/songs/likes  updates like to song
        $.post(`/event/addLike/${eventId}/${guestId}/${songId}`, function (
          data,
          status
        ) {
          song.likecounter += 1;
        ...
        // user remove like
        // /POST /event/songs/likes  updates like to song
        $.post(`/event/removeLike/${eventId}/${guestId}/${songId}`, function (
          data,
          status
        ) {
          song.likecounter -= 1;
...
$.get("/guest/getGuestData/" + eventId + "/" + guestId, function (
  data,
  status
) {
...
$.get(`/event/songsLikedByUser/${eventId}/${guestId}`, function (
  Songsdata,
  status
) {
...
function sendSong() {
  ...
  $.post("/event/addSong/" + eventId + "/" + songId, function (
    songDetails,
    status
  ) {
